...
export class EchelonComponent implements OnInit {
  ...
  constructor(private echelonService: EchelonsService, private http: HttpClient, private _formBuilder: FormBuilder, private auth: AuthService, public dialog: MatDialog, private router: Router) {
  ...
  loadTable() {
    ...
    this.echelonService.getDisplayedDataFromApiCall().subscribe( ( d )=> {
      this._data = d
  ...
  addNewGrade(){
    ...
    this.isLoading= true;
    this.http.post("/api/echelons", { id, code, libelle, etat }).subscribe( ( d ) => {
      this.loadTable()
