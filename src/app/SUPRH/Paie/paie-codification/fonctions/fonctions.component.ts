...
export class FonctionsComponent implements OnInit {
  ...
  constructor(private fonctionService: FonctionsService, private http: HttpClient, private _formBuilder: FormBuilder, private auth: AuthService, public dialog: MatDialog, private router: Router) {
  ...
  loadTable() {
    ...
    this.fonctionService.getDisplayedDataFromApiCall().subscribe( ( d )=> {
      this._data = d
  ...
  addNewGrade(){
    ...
    this.isLoading= true;
    this.http.post("/api/fonctions", { id, code, libelle, etat }).subscribe( ( d ) => {
      this.loadTable()
