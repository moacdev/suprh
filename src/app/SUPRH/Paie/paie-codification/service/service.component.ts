...
export class ServiceComponent implements OnInit {
  ...
  constructor(private servicesService: ServicesService, private http: HttpClient, private _formBuilder: FormBuilder, private auth: AuthService, public dialog: MatDialog, private router: Router) {
  ...
  loadTable() {
    ...
    this.servicesService.getDisplayedDataFromApiCall().subscribe( ( d )=> {
      this._data = d
  ...
  addNewGrade(){
    ...
    let direction = this.firstFormGroup.value.directionCtrl;
    
    this.isLoading= true;
    this.http.post("/api/services", { id, code, libelle, etat, direction }).subscribe( ( d ) => {
      this.loadTable()
